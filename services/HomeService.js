...
function buildCarousel(carousel = []) {
  const order = ["us_market", "crypto", "commodities", "sectors", "sentiment"];
...
function formatCarouselItems(items, cardId) {
  ...
  const formatted = items
    .filter((i) => {
      ...
      // remove empty sector values like Energy, Consumer, Health with no %
      if (cardId === "sectors") {
    ...
    .map((i) => {
      const label = cardId === "sectors" ? shortenSector(i.label) : i.label;
      const val = String(i.value ?? "").trim();
      ...
      return val ? `${label} ${indicator}${val}` : `${label}`;
    });

  if (!formatted.length) return "No sector data";
