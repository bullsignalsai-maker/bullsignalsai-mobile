...
export async function getTechnicalDetail(symbol) {
  ...
  const findIndicator = (arr = [], name = "") =>
    Array.isArray(arr)
      ? arr.find((x) => String(x?.name || "").toLowerCase().includes(name.toLowerCase()))
      : null;
  ...
  return {
    ...
    technical: {
      ...
      trend: {
        ...
        trend_strength_20: safeNum(trend?.trend_strength_20 ?? trendStrength?.value),
        comment: trend?.comment || trendStrength?.comment || null,
      },

      volume: {
        label: volume?.label || null,
        volume_vs_ma20_pct: safeNum(volume?.volume_vs_ma20_pct ?? volumeVsMa20?.value),
    ...
    featuresMeta: {
        ...
        volatility_20d: safeNum(raw?.featuresMeta?.volatility_20d ?? volatility?.volatility_20d),
