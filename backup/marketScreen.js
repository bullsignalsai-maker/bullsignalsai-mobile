          ...
          <View style={styles.overviewCard}>
              ...
              return (
                <View key={item.label} style={styles.tableRow}>
                  <Text style={styles.colSymbol}>
                    {item.label.includes("SPY") ? "SPY" : "QQQ"}
                  </Text>
            ...
            {carousel.find(c => c.id === "crypto")?.items
              ...
              .map(item => {
                ...
                return (
                  <View key={item.label} style={styles.tableRow}>
                    <Text style={styles.colSymbol}>{item.label}</Text>
                    ...
                    <Text style={[styles.colChange, { color: up ? "#00E396" : "#EF4444" }]}>
                      {up ? "▲" : "▼"} {Math.abs(q.price * q.changePct / 100).toFixed(2)}
                    </Text>
            ...
            {/* ===== COMMODITIES (ETFs clearly labeled) ===== */}
            ...
            {carousel.find(c => c.id === "commodities")?.items?.map(item => {
              ...
              return (
                <View key={item.label} style={styles.tableRow}>
                  <Text style={styles.colSymbol}>
                    {item.label.includes("GLD") ? "Gold (GLD)" :
                    item.label.includes("USO") ? "Oil (USO)" :
                    "Silver (SLV)"}
                  </Text>
            ...
            {/* ===== SECTORS ===== */}
            <Text style={styles.sectionSubtle}>Sectors</Text>

            {carousel.find(c => c.id === "sectors")?.items?.map(item => {
