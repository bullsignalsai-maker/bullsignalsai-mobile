...
export default function FullTechnicalDetailScreen({ route }) {
 ...
 /* ========= NARRATIVE (ADD-ONLY) ========= */
  ...
  const onShare = useCallback(async () => {
    try {
      const lines = [
        `BullSignalsAI — Technical Snapshot`,
        ``,
        `${symbol || "—"} • ${companyName || ""}`.trim(),
        `Price: ${money(quote?.current)} (${quote?.changePct == null ? "—" : `${quote.changePct >= 0 ? "+" : ""}${quote.changePct.toFixed(2)}%`})`,
        ``,
        `Trend: ${derived.trendLabel} (Strength: ${derived.trendStrength == null ? "—" : derived.trendStrength.toFixed(2)})`,
        `Price vs SMA20: ${derived.priceVsSMA20 == null ? "—" : `${derived.priceVsSMA20.toFixed(2)}%`}`,
        ``,
        `RSI: ${derived.rsi == null ? "—" : derived.rsi.toFixed(1)}`,
        `MACD: ${derived.macd == null ? "—" : derived.macd.toFixed(2)} | Signal: ${derived.macdSignal == null ? "—" : derived.macdSignal.toFixed(2)} | Hist: ${derived.macdHist == null ? "—" : derived.macdHist.toFixed(2)}`,
        ``,
        `Volume vs MA20: ${derived.volumeVsMA20 == null ? "—" : `${derived.volumeVsMA20.toFixed(2)}%`}`,
        `Volatility20D: ${derived.volatility20d == null ? "—" : derived.volatility20d.toFixed(2)} | ATR14: ${derived.atr14 == null ? "—" : derived.atr14.toFixed(2)}`,
        ``,
        `Returns: 1D ${derived.ret1d == null ? "—" : `${derived.ret1d.toFixed(2)}%`} | 5D ${derived.ret5d == null ? "—" : `${derived.ret5d.toFixed(2)}%`} | 10D ${derived.ret10d == null ? "—" : `${derived.ret10d.toFixed(2)}%`}`,
        ``,
        `SMA: 20 ${derived.sma20 == null ? "—" : derived.sma20.toFixed(2)} | 50 ${derived.sma50 == null ? "—" : derived.sma50.toFixed(2)} | 200 ${derived.sma200 == null ? "—" : derived.sma200.toFixed(2)}`,
        ``,
        `Candlestick: Body ${derived.bodyPct == null ? "—" : `${derived.bodyPct.toFixed(2)}%`} • Upper wick ${derived.upperWickPct == null ? "—" : `${derived.upperWickPct.toFixed(2)}%`} • Lower wick ${derived.lowerWickPct == null ? "—" : `${derived.lowerWickPct.toFixed(2)}%`} • Range ${derived.rangePct == null ? "—" : `${derived.rangePct.toFixed(2)}%`} • Gap ${derived.gapPct == null ? "—" : `${derived.gapPct.toFixed(2)}%`}`,
        ``,
      ...
      await Share.share(
        {
          title: "BullSignalsAI — Technical Snapshot",
          message: lines.join("\n"),
        },
        Platform.OS === "ios"
          ? { subject: "BullSignalsAI — Technical Snapshot" }
          : undefined
      );
    } catch (e) {
  ...
  }, [symbol, companyName, quote, derived]);

  /* ================= RENDER ================= */
  const headerPrice = quote?.current ?? null;
